...
const togglePlus = inject<() => void>('togglePlus');
...
const onSubmit = async () => {
  if (userStore.user?.is_subscribed) {
    toast({
      ...
      description: 'Сначала отмените действующую подписку или дождитесь её окончания',
      variant: 'destructive',
    });
    return;
  }
  try {
    await userStore.subscribe(props.duration);

    if (props.duration === 30 && togglePlus) {
      togglePlus();
    } else if (toggle) {
      toggle();
    }
...
<template>
  <Card>
    <CardHeader>
      ...
      <CardDescription>
        Заполните платежные данные ниже для того чтобы оформить премиум.
      </CardDescription>
    </CardHeader>
    <CardContent class="grid gap-6">
      ...
      <div class="grid grid-cols-3 gap-4">
        <div class="grid gap-2">
          <Label for="month">Срок действия</Label>
          <Select>
        ...
        <div class="grid gap-2">
          ...
          <Select>
            ...
            <SelectContent>
              <SelectItem v-for="i in 20" :key="i" :value="`${new Date().getFullYear() - 10 + i}`">
    ...
    <CardFooter>
      <Button :disabled="isLoading" @click="onSubmit" class="w-full"> Продолжить </Button>
    </CardFooter>
  </Card>
</template>
