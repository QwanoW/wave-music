  ...
  <section class="w-full h-full flex flex-col gap-y-1 group transition-all">
    <div @click="href && router.push(href)" class="w-full h-full relative">
      <img
        ...
        class="w-full h-full object-cover aspect-square duration-200 group-hover:brightness-[0.25]"
        :src="API_URL + cover"
        alt="" />
      <button
    ...
    <div :class="rounded ? 'items-center' : ''" class="w-full flex flex-col justify-center">
      <h1 class="text-xl font-bold text-amber-700">{{ title }}</h1>
      <p v-if="description" class="text-sm font-medium text-amber-700">{{ description }}</p>
      <p v-if="number" class="text-sm font-medium text-amber-700">{{ number }}</p>
    </div>
  </section>
</template>
